...
def _rotary_kernel(
    ...
    max_total_len,
    HEAD_Q, HEAD_K,  # N_CTX 代表要计算的上下文长度
    ...
    q0 = tl.load(Q + off_q0, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < HEAD_Q), other=0.0)
    q1 = tl.load(Q + off_q1, mask=(cur_seq_range[:, None, None] < max_total_len) & (cur_head_range[None, :, None] < HEAD_Q), other=0.0)
...
def rotary_emb_fwd(q, k, cos, sin):
    ...
    _rotary_kernel[grid](
        ...
        total_len, head_num_q, head_num_k,
